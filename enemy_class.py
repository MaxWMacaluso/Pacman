...
class Enemy:
    ...
    def findNextPos(self,player_target):
        vec_pos = self.BreadthFirstSearchEnemyTarget([int(self.grid_pos.x), int(self.grid_pos.y)], [int(player_target[0]), int(player_target[1])])[1]
...
class RedEmemy(Enemy):
    def draw(self):
        pygame.draw.circle(self.app.screen, (255, 0, 0), (int(self.pix_pos.x), int(self.pix_pos.y)), self.radii)


class PinkEmemy(Enemy):
    def draw(self):
        pygame.draw.circle(self.app.screen, (255, 184, 255), (int(self.pix_pos.x), int(self.pix_pos.y)), self.radii)


class EnemyFactory():
    def CreateEnemy(self,type):
        if(type == "Blue"):
            return BlueEnemy()
        elif(type=="Orange"):
            return OrangeEnemy()
